...
def main():
    ...
    ################ Regret
    regret_indist = np.load(os.path.join(args.indist_dir, "NLL_IC.npy"))
    regret_ood = np.load(os.path.join(args.ood_dir, "NLL_IC.npy"))
    ...
    plt.legend(loc="upper right")
    plt.savefig(os.path.join(output_dir, f"nll_ic_indist_{args_indist.dataset}_ood_{args_ood.dataset}.jpg"))
    plt.close()
    ...
    plot_roc_curve(fpr, tpr, output_dir, label="IC")

    rocauc = metrics.auc(fpr, tpr, )
    print('AUC for likelihood scaled with input complexity is: ', rocauc)
    ...
    plot_roc_curve(fpr, tpr, output_dir, label="NLL_IC")
